...
(module MR racket
  ...
  (define itp:pgm
    (match-lambda
      [(or `(program ,pi (define (,_ ...) : ,_ ,_ ,e) . ,def*)
           `(program ,pi ,def* ... ,e))
    ...
    (lambda (env expr cont)
      (let ([rec (curry itp:exp env)])
        (match expr
          ...
          [(or `(let ([,r* ,e*]...) ,eb))
           (let loop ([r* r*] [e* e*] [env env])
             (if (empty? r*)
               (itp:exp env eb cont)
               (itp:exp env (car e*)
                 (lambda (v) (loop (cdr r*) (cdr e*) (env-add env (car r*) v))))))]
          [(or `(bind [,re ,ee] ,eb))
           (rec ee (lambda (ve) (itp:exp (env-add env re ve) eb cont)))]
          [(or `(if ,e1 ,e2 ,e3))
           (rec e1 (lambda (v1) (if v1 (rec e2 cont) (rec e3 cont))))]
          [(or `(begin ,e* ... ,eb))
           (cps-map rec e* (lambda (v*) (rec eb cont)))]
  ...
  (define def->f.c
    (match-lambda
      [(or `(define (,f [,v* : ,_]...) : ,_ ,_ ,e) 
           `(define (,f [,v* : ,_]...) : ,_ ,e))
       (make-pair f `(closure ,v* ,e #f))]))
...
;      (match stmt
;        [`(assign ,r ,e)
;          (itp:exp env e (lambda (v) (cont (env-set env r v))))]
;        [`(collect ,i)
;          (cont env)])))
